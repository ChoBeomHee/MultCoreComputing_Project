...
int main(int argc, char* argv[])
{
	...
	clock_t start, end;		// 시작시간과 끝 시간을 기록하기 위한 변수
	double run_time;		// 런타임을 저장할 변수
...
#pragma omp parallel for private(x), private(y) // x, y는 독립변수이므로 private 처리
	for (x = 0; x < DIM; x++)
		for (y = 0; y < DIM; y++) kernel(x, y, temp_s, bitmap);

	end = clock();			// 끝 시간 기록
	run_time = ((double)(end - start)) / CLOCKS_PER_SEC;	// 실행시간 기록

	printf("OpenMP (%d threads) ray tracing: %.3f sec\n", no_threads, run_time);		// 결과 출력
